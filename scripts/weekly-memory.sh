#!/usr/bin/env bash
# weekly-memory.sh — Merged: weekly memory update + weekly digest
# Runs every Sunday at 09:00 SAST via LaunchAgent.
# 1. Consolidates 7 days of daily memory logs → updates MEMORY.md → git commits
# 2. Pulls Supabase interaction data → multi-model analysis → sends Telegram digest to Josh
set -uo pipefail
...
# Source task helpers if available
[[ -f "$WS/scripts/lib/task-helpers.sh" ]] && source "$WS/scripts/lib/task-helpers.sh"

LOG="$WS/out/weekly-memory.log"
mkdir -p "$WS/out" "$WS/memory"

log() { echo "[$(date '+%H:%M:%S')] $*" | tee -a "$LOG"; }
log "=== Weekly Memory + Digest starting ==="

TASK_ID=""
if command -v task_create &>/dev/null; then
  TASK_ID=$(task_create "Weekly Memory + Digest" "Distilling 7 days of logs into MEMORY.md + sending weekly digest" "weekly-memory" "normal" 2>/dev/null || echo "")
fi

# ============================================================================
# PART 1: MEMORY.md UPDATE
# Reads last 7 days of daily logs, extracts long-term insights via Sonnet,
# updates MEMORY.md, git commits.
# ============================================================================

MEMORY_FILE="$WS/memory/MEMORY.md"

log "Part 1: Gathering daily logs..."

LOG_CONTENT=""
for i in 1 2 3 4 5 6 7; do
  D=$(date -v -${i}d '+%Y-%m-%d' 2>/dev/null || date -d "-${i} days" '+%Y-%m-%d' 2>/dev/null || echo "")
  [[ -z "$D" ]] && continue
  LOG_FILE="$WS/memory/${D}.md"
  if [[ -f "$LOG_FILE" ]]; then
...
if [[ -n "$LOG_CONTENT" ]]; then
  log "  Extracting insights via Sonnet..."
...
PROMPT
  ...
  INSIGHTS=$(/Users/henryburton/.openclaw/bin/claude-gated --print --model claude-sonnet-4-6 < "$PROMPT_TMP" 2>/dev/null || echo "")
  rm -f "$PROMPT_TMP"
...
MEMORY
    ...
    git -C "$WS" add memory/MEMORY.md 2>/dev/null || true
    git -C "$WS" diff --staged --quiet 2>/dev/null || \
      git -C "$WS" commit -m "Weekly memory update $(date '+%Y-%m-%d')" --no-verify 2>/dev/null || true
    log "  Git committed"
  else
    log "  No insights extracted — MEMORY.md unchanged"
  fi
else
  log "  No daily logs found for last 7 days — skipping MEMORY.md"
fi

# ============================================================================
# PART 2: WEEKLY DIGEST (multi-model analysis → Telegram)
# Pulls interaction_log + user_models + agent_memory from Supabase,
# runs Haiku → Opus → o3 → GPT-5.2 pipeline, sends to Josh on Telegram.
# ============================================================================

log "Part 2: Weekly digest pipeline..."
...
if [[ -z "$KEY" ]]; then
  log "ERROR: SUPABASE_SERVICE_ROLE_KEY not set — skipping digest"
  [[ -n "$TASK_ID" ]] && command -v task_complete &>/dev/null && task_complete "$TASK_ID" "MEMORY.md updated but digest skipped (no key)" 2>/dev/null || true
  exit 0
fi

export KEY SUPABASE_URL BOT_TOKEN CHAT_ID WS
...
# ── Task completion ──────────────────────────────────────────────────────────
if [[ -n "$TASK_ID" ]] && command -v task_complete &>/dev/null; then
  task_complete "$TASK_ID" "MEMORY.md updated and weekly digest sent" 2>/dev/null || true
fi
